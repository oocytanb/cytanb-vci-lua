...
-- `cytanb.lua` モジュールのアノテーションです。

--- @alias cytanb_uuid_t table

--- @class cytanb_module @UUID、ログ、色、メッセージなど、基礎的な機能を提供するモジュール。
...
--- @field ListToMap fun (list: table, itemValue: any): table @リストをテーブルに変換する。リストの要素の値がキー値となる。`itemValue` に要素の値を指定する。`itemValue` に `nil` を指定するか省略した場合は、リストの要素の値が使われる。
...
--- @field EmitMessage fun (name: string, parameterMap: table<string, any>) @パラメーターを JSON シリアライズして `vci.message.Emit` する。`parameterMap` は省略可能。
--- @field OnMessage fun (name: string, callback: fun(sender: table, name: string, parameterMap: table)) @メッセージハンドラを登録する。
cytanb = {}
