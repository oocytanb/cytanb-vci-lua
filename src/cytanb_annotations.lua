...
---@field EmitCommentMessage fun (message: string, senderOverride: table) @**EXPERIMENTAL:実験的な機能。** スタジオ内に、疑似的にコメントメッセージ (メッセージ名: `comment`) を送信する。`message` に、送信するメッセージの内容を指定する。`senderOverride` に、送信者情報を置換するためのテーブルを指定する。(例: `{name = 'NicoUser', commentSource = 'Nicolive'}`) 受信側は `OnCommentMessage` を使用すること。
---@field OnCommentMessage fun (callback: fun(sender: table, name: string, message: string)) @**EXPERIMENTAL:実験的な機能。** コメントメッセージ (メッセージ名: `comment`)を受信するコールバック関数を登録する。`EmitCommentMessage` で送信したメッセージに対応できる。
---@field EmitNotificationMessage fun (message: string, senderOverride: table) @**EXPERIMENTAL:実験的な機能。** スタジオ内に、疑似的に通知メッセージ (メッセージ名: `notification`) を送信する。`message` に、送信するメッセージの内容を指定する。(`joined` | `left`)。`senderOverride` に、送信者情報を置換するためのテーブルを指定する。(例: `{name = 'FooUser'}`) 受信側は `OnNotificationMessage` を使用すること。
---@field OnNotificationMessage fun (callback: fun(sender: table, name: string, message: string)) @**EXPERIMENTAL:実験的な機能。** 通知メッセージ (メッセージ名: `notification`)を受信するコールバック関数を登録する。`EmitNotificationMessage` で送信したメッセージに対応できる。
