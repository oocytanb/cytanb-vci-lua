...
---@field EmitMessage fun (name: string, parameterMap: table<string, any>) @パラメーターを `TableToSerializable` および `json.serialize` でシリアライズして `vci.message.Emit` する。`name` はメッセージ名を指定する。`parameterMap` は送信するパラメーターのテーブルを指定する(省略可能)。また、`InstanceID` がパラメーターフィールド `__CYTANB_INSTANCE_ID` として付加されて送信される。
---@field OnMessage fun (name: string, callback: fun(sender: table, name: string, parameterMap: table)) @`EmitMessage` したメッセージを受信するコールバック関数を登録する。`name` はメッセージ名を指定する。`callback` 関数に渡される `parameterMap` は `json.parse` および `TableFromSerializable` でデシリアライズしたテーブル。また、パラメーターフィールド `__CYTANB_INSTANCE_ID` を利用してメッセージ送信元のインスタンスを識別可能。もし `cytanb.EmitMessage` を通して送信されたデータでなければ、パラメーターフィールド `__CYTANB_MESSAGE_VALUE` に値がセットされる。**EXPERIMENTAL:実験的な機能として、受信したデータにパラメーターフィールド `__CYTANB_MESSAGE_SENDER_OVERRIDE` が指定されていた場合は、送信者情報置換処理を行い、パラメーターフィールド `__CYTANB_MESSAGE_ORIGINAL_SENDER` にオリジナルの送信者情報がセットされる。**
